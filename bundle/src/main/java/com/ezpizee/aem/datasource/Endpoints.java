...
public class Endpoints extends WCMUsePojo {
    ...
    public void activate() throws Exception {
        ...
        for (String service : data.keySet()) {
            JsonObject actions = data.get(service).getAsJsonObject();
            for (String action : actions.keySet()) {
                vm = new ValueMapDecorator(new HashMap<>());
                String endpoint = actions.get(action).getAsJsonObject().get("uri").getAsString();
