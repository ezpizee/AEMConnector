package com.ezpizee.aem.utils.detection;
...
public class UserAgentDetector implements IUserAgentDetector {
    ...
    static java.util.Locale getLocaleSecondPass(UserAgentContext context, UserAgentDetectionResult result) {
        if (result.getBrowser().getFamily() == BrowserFamily.FIREFOX || result.getBrowser().getFamily() == BrowserFamily.OTHER_GECKO) {
            if (result.getOperatingSystem().getFamily() == OSFamily.MACOSX) {
                if (context.consume("ja-JP-mac", MatchingType.EQUALS, MatchingRegion.PARENTHESIS)) {
                    return new java.util.Locale(Language.JA.getLabel(), Country.JP.getLabel());
                }
            }
        }
    ...
    static java.util.Locale getLocale(String s, UserAgentContext context) {
        if (s.equals("LG")) {
        ...
        if (s.length() == 4 && s.charAt(0) == '[' && s.charAt(3) == ']') {
            ...
            if (l != null) {
                return new java.util.Locale(l.getLabel());
            }
        } else if (s.length() == 2) {
            ...
            if (l != null && c != null) {
                return new java.util.Locale(l.getLabel(),c.getLabel());
            }
    ...
    static java.util.Locale getLocale(UserAgentContext context) {
        //String ua = context.getUA();
        ...
        java.util.Locale defaultLocale = new java.util.Locale(Language.UNKNOWN.getLabel());
        String toBeConsumedInPar = null;

        Iterator<String> it = context.getRegularTokensIterator();
        while (it.hasNext()) {
            String s = it.next();
            java.util.Locale l = getLocale(s,context);
            if (l != null) {
            ...
            if (s.equals("MSN Optimized") || s.equals("msn OptimizedIE8") || s.matches("BO[0-9]?IE[89](_v[0-9]+)?")) {
                ...
                if (afterMsnOpt) {
                    if (s.length() == 2) {
                        ...
                        if (c!=null) {
                            toBeConsumedInPar = s;
                            defaultLocale = new java.util.Locale(Language.UNKNOWN.getLabel(), c.getLabel());
                        }
                    } else if (s.length()==4) {
                        Language l = LocaleHelper.getLanguage(s.substring(0,2));
                        Country c = LocaleHelper.getCountry(s.substring(2,4));
                        if (l != null && c != null) {
                            toBeConsumedInPar = s;
                            defaultLocale = new java.util.Locale(l.getLabel(),c.getLabel());
                        }
                    }
                } else {
                    java.util.Locale l = getLocale(s,context);
    ...
    public UserAgentDetectionResult parseUserAgent(String ua) {
        ...
        if (ua == null || ua.length()<3) {
            ...
            res.setLocale(new Locale(Language.UNKNOWN.getLabel()));
